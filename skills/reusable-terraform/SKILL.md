...
## Brownfield imports — defer to `import-env` first

The "reusable asset" framing above assumes the module will create infrastructure. A module
generated by importing **existing** infrastructure is the opposite case: it represents one
live environment, is not meant to launch a second one, and its inputs exist to *update*
resources that already exist.

If the module came from an import (literal-only values, names matching live resources,
no `variables.tf`), **invoke the `import-env` skill first**. It settles whether the user
wants in-place management or a digital twin — and only the digital-twin track wants the full
parameterization pass described below.
