...
already-existing, already-running resource brought under Torque management) is a different,
equally valid category with the opposite defaults — not a lesser or unfinished version of a
normal module. Full reasoning: `import-cloud-resources-as-environment`'s
...
- **Rule 3 (avoid over-parameterization) becomes even stricter.** The default is zero variables.
  Only add an input if the user explicitly wants to be able to *update* the live resource through
  Torque (instance size, tags, replica counts) — never to make the module launchable as a second,
  independent copy. If a field is pinned by `lifecycle.ignore_changes` or is creation-time-only, an
  input wired to it is inert against the live resource; say so in the input's description rather
  than presenting it as an effective one.
